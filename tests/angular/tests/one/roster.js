...
(function() {
    ...
    Roster.$factory = [
        ...
        function($timeout, Resource, Unit) {
            _.extend(Roster, {
                $$resource: new Resource('http://127.0.0.1:3001/api/collection'),
                $timeout: $timeout,
                $Unit: Unit
            });
    ...
    Roster.prototype.$getUnits = function() {
        ...
        return this.$futureRosterData.get('data').then(function(rosterUnits) {
            var units = _.reduce(rosterUnits, function(a, rosterUnit) {
                var unit = Roster.$Unit.$find(rosterUnit);
    ...
    Roster.prototype.$saveUnits = function() {
        ...
        return Roster.$$resource.set(this.id, {
            units: units
        });
    };
