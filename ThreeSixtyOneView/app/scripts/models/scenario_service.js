...
.service('ScenarioService', ["$q", "$rootScope", "ScenarioModel", "ProjectsService", "CONFIG", "EVENTS", "Model", function ($q, $rootScope, ScenarioModel, ProjectsService, CONFIG, EVENTS, Model) {
	var MyScenarioModel,
		myScenarios,
		self = this,
		temporaryScenarioProject,
		temporaryScenario,
		temporaryScenarioBase,
		temporaryScenarioType;
	...
	this.setTemporaryScenario = function addTemporaryScenario(project, scenario, baseScenario, type) {
		temporaryScenarioProject = project;
		temporaryScenario = scenario;
		temporaryScenarioBase = baseScenario;
		temporaryScenarioType = type;
	};

	this.getTemporaryScenario = function getTemporaryScenario() {
		return {
			project: temporaryScenarioProject,
			scenario: temporaryScenario,
			baseScenario: temporaryScenarioBase,
			type: temporaryScenarioType
		};
	};

	this.removeTemporaryScenario = function removeTemporaryScenario() {
		temporaryScenarioProject = undefined;
		temporaryScenario = undefined;
		temporaryScenarioBase = undefined;
		temporaryScenarioType = undefined;
	};
}]);
