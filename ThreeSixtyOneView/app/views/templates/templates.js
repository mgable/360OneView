angular.module('ThreeSixtyOneView').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/directives/display_actions.tpl.html',
    ...
    "\n" +
    "\t\t\t<li ng-repeat=\"item in CONFIG.filterMenu.items\" class=\"header\" ms-link=\"{{item.label}}\" ng-click=\"setFilter(item.filterType, item, true)\">\r" +
    "\n" +
    "\t\t\t\t <a>{{item.label}}</a>\r" +
    "\n" +
    "\t\t\t</li>\r" +
    ...
    "\n" +
    "\t\t<button ng-disabled=\"selectedItem.isMaster\" class=\"btn btn-default\" ng-click=\"create(CONFIG.displayActionsCreate)\" data-ms-id='createButton'><icon type=\"plus\"></icon>CREATE</button>\r" +
    "\n" +
    "\t</div>\r" +
    "\n" +
    "</div>\r" +
  ...
  $templateCache.put('views/directives/inline_rename.tpl.html',
    ...
    "\n" +
    "    \t<input type=\"text\" class=\"title\" ng-model=\"item.title\" required ng-maxlength=\"256\" ng-minlength=\"2\" ng-pattern='inputRestrictions.characterRestrictions' tabindex=\"1\" />&nbsp;\r" +
    "\n" +
