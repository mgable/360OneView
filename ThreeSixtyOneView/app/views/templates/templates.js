angular.module('ThreeSixtyOneView').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/modal/all_views.tpl.html',
    ...
    "\t\t\t</div>\n" +
    "\t\t\t<div class=\"list-box\">\n" +
    "\t\t\t\t<div ng-repeat=\"view in getViewsList() | filter:searchTerm | orderBy:'auditInfo.lastUpdatedOn':true\" class=\"item clickable\" ng-class=\"{'selected': view.id === selectedView.id}\" ng-click=\"selectedView.id = view.id\">\n" +
  ...
  $templateCache.put('views/modal/scenario_analysis_element_files.tpl.html',
    ...
    "\t\t\t\t\t\t\t<icon type=\"circle-o\" cname=\"circle\"></icon><icon type=\"dot-circle-o\" cname=\"dot-circle\"></icon>{{file.name}}\n" +
    "\t\t\t\t\t\t</div>\n" +
    "\t\t\t\t\t\t<div class=\"item-meta\">\n" +
  ...
  $templateCache.put('views/modal/scenario_create.tpl.html',
    ...
    "\t\t\t\t\t\t<input type=\"text\" focus placeholder=\"Enter Scenario Name\" required ng-maxlength=\"256\" ng-minlength=\"2\" ng-pattern='inputRestrictions.characterRestrictions' validator=\"isScenarioTitleUnique\" error-type=\"isUnique\" ng-model=\"scenario.title\" data-ms-id=\"ScenarioCreate.inputName\"/>\n" +
    ...
    "\t\t\t\t\t\t\t<accordion-group is-open=\"true\" ng-hide=\"hideMasterProject(masterProject.title, masterProjectReferenceScenario.title, searchText)\">\n" +
    ...
    "\t\t\t\t\t\t\t\t<accordion-heading>{{project.title}} Project</accordion-heading>\n" +
  ...
  $templateCache.put('views/modal/scenario_templates.tpl.html',
    "<p>This is the modal/scenario_templates.tpl view.</p>\n"
  );
  ...
  $templateCache.put('views/modal/simple_input.tpl.html',
    ...
    "\t\t\t\t\t\t<input type=\"text\" class=\"form-control\" id=\"inputField\" ng-model=\"item.title\" focus required ng-maxlength=\"256\" ng-minlength=\"2\" ng-pattern='inputRestrictions.characterRestrictions'/>\n" +
    "\t\t\t\t\t</label>\n" +
    "\t\t\t\t</div>\n" +
    "\t\t\t</form>\n" +
    "\t\t</div>\n" +
