angular.module('ThreeSixtyOneView').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/directives/inline_description.tpl.html',
    ...
    "\n" +
    "\t<textarea ng-disabled=\"!isActive\" ng-maxlength=\"256\" ng-pattern='/^[^\\\\\\/\\?\\:\\*\"><|]+$/' ng-model=\"item.description\" ng-class=\"{'active': isActive}\" class=\"description inputTarget\"></textarea>\n" +
    "</form>"
  );
  ...
  $templateCache.put('views/directives/ms_dropdown.tpl.html',
    ...
    "\t\t<ul>\n" +
    "\t\t\t<li class=\"ms-item selectSort\" ng-class=\"{disabled:sortOrderItem.label === selectedSortOrderItem.label}\" ng-repeat=\"sortOrderItem in sortOrderItems\" ng-click=\"selectSortOrder(sortOrderItem)\"><icon type=\"check\" cname=\"ms-ok\"></icon>{{sortOrderItem.label}}</li>\n" +
    "\t\t</ul>\n" +
