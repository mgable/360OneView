angular.module('ThreeSixtyOneView').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/directives/flipbook.tpl.html',
    ...
    "\n" +
    "\t<div ng-transclude></div><button class=\"btn btn-default btn-sm\" ng-click=\"backward()\" ng-disabled=\"isDisabled()\">back</button>&nbsp;<button class=\"btn btn-default btn-sm\" ng-click=\"forward()\" ng-disabled=\"isDisabled(DIRECTION)\">{{label}}</button>\r" +
    "\n" +
    "</div>"
  );
  ...
  $templateCache.put('views/modal/all_views.tpl.html',
    ...
    "\n" +
    "\t<h3 class=\"subtitle\">Select A {{subTitle}}</h3>\r" +
    "\n" +
    "</div>\r" +
    "\n" +
    "<div class=\"body\">\r" +
    ...
    "\n" +
    "\t\t\t\t<div ng-repeat=\"view in getViewsList() | filter:searchTerm | orderBy:'auditInfo.lastUpdatedOn':true\" class=\"item clickable\" ng-class=\"{'selected': view.id === selectedView.id}\" ng-click=\"selectedView.id = view.id\">\r" +
    "\n" +
  ...
  $templateCache.put('views/modal/scenario_analysis_element_files.tpl.html',
    "<div data-ms-id=\"analysisElementReplace\">\r" +
    "\n" +
    "\t<div class=\"header\">\r" +
    "\n" +
    "\t\t<h4 class=\"title\">{{selectedScenarioElement.cubeMeta.label}}</h4>\r" +
    ...
    "\n" +
    "\t\t\t\t\t\t\t<icon type=\"circle-o\" cname=\"circle\"></icon><icon type=\"dot-circle-o\" cname=\"dot-circle\"></icon>{{file.name}}\r" +
    "\n" +
    "\t\t\t\t\t\t</div>\r" +
    "\n" +
    "\t\t\t\t\t\t<div class=\"item-meta\">\r" +
  ...
  $templateCache.put('views/modal/simple_input.tpl.html',
    ...
    "\n" +
    "\t\t\t\t\t\t<input type=\"text\" class=\"form-control\" id=\"inputField\" ng-model=\"item.title\" focus required ng-maxlength=\"256\" ng-minlength=\"2\" ng-pattern='inputRestrictions.characterRestrictions'/>\r" +
    "\n" +
    "\t\t\t\t\t</label>\r" +
    "\n" +
    "\t\t\t\t</div>\r" +
