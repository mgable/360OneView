angular.module('ThreeSixtyOneView').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/directives/member.tpl.html',
    ...
    "\t<span class=\"expand-handle clickable\" ng-if=\"hasMembers()\" ng-click=\"expanded[member.label] = !expanded[member.label]\">\n" +
  ...
  $templateCache.put('views/modal/filter_selection.tpl.html',
    ...
    "<div class=\"body\">\n" +
    "\t<div class=\"side-menu\">\n" +
    "\t\t<div class=\"menu-item clickable\" ng-repeat=\"cat in dimensions\" ng-click=\"chooseFilter(cat, false, false)\" ng-class=\"{active: cat.label == selectedFilter.cat.label}\">\n" +
    "\t\t\t<div>{{cat.label}}\n" +
    "\t\t\t\t<span>({{categorizeValuesCount($index, addedFilter[cat.label]).selected}}/{{categorizedValue[$index].total}})</span>\n" +
    "\t\t\t</div>\n" +
    "\t\t\t<div ng-show=\"categorizedValue[$index].selected < categorizedValue[$index].total\" class=\"values text-holder\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"{{categorizedValue[$index].label.join(', ')}}\">{{categorizedValue[$index].label.join(', ')}}</div>\n" +
    "\t\t\t<div ng-hide=\"categorizedValue[$index].selected < categorizedValue[$index].total\" class=\"values text-holder\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"{{categorizedValue[$index].label.join(', ')}}\">All</div>\n" +
    "\t\t</div>\n" +
    "\t</div>\n" +
    "\t<div class=\"content\">\n" +
    ...
    "\t\t\t\t: <span ng-repeat=\"missing in emptyFiltersList\">\n" +
    ...
    "\t\t\t\t\t\t</div>\n" +
    "\t\t\t\t\t\t<div class=\"dropdown-menu\" ms-link-group radio=\"true\" selected-item=\"{{selectedFilter.selFil.label}}\">\n" +
    "\t\t\t\t\t\t\t<div ng-repeat=\"item in selectedFilter.cat.members\" class=\"menu-item clickable\" ms-link=\"{{item.label}}\" ng-click=\"chooseFilter(selectedFilter.cat, false, $index)\">\n" +
    "\t\t\t\t\t\t\t\t{{item.label.toLowerCase()}}\n" +
    "\t\t\t\t\t\t\t</div>\n" +
    "\t\t\t\t\t\t</div>\n" +
    "\t\t\t\t\t</div>\n" +
    "\t\t\t\t</div>\n" +
    "\t\t\t\t<div class=\"search-box\" ng-show=\"selectedFilter.selFil\">\n" +
    "\t\t\t\t\t<icon type=\"filter\"></icon>\n" +
    "\t\t\t\t\t<input type=\"text\" placeholder=\"Filter List\" ng-model=\"filterSearch.label\" ng-keyup=\"searchFilters(selectedFilter.selFil, filterSearch)\">\n" +
    "\t\t\t\t</div>\n" +
    "\t\t\t</div>\n" +
    "\t\t\t<div class=\"selection-tools\">\n" +
    "\t\t\t\t<div class=\"dropdown\">\n" +
    "\t\t\t\t\t<div class=\"static-button\" ng-hide=\"filterCount.total === 0\">\n" +
    "\t\t\t\t\t\t<div class=\"selection-toggle clickable\" ng-show=\"filterCount.selected === 0\" ng-click=\"selectFilters(selectedFilter.cat.label, true, true);\"><icon type=\"square-o\"></icon></div>\n" +
    "\t\t\t\t\t\t<div class=\"selection-toggle clickable blue\" ng-show=\"filterCount.selected === filterCount.total\" ng-click=\"selectFilters(selectedFilter.cat.label, true, false);\"><icon type=\"check-square\"></icon></div>\n" +
    "\t\t\t\t\t\t<div class=\"selection-toggle clickable blue\" ng-show=\"filterCount.selected % filterCount.total > 0.01\" ng-click=\"selectFilters(selectedFilter.cat.label, true, true);\"><icon type=\"minus-square\"></icon></div>\n" +
    "\t\t\t\t\t\t<div class=\"clickable dropdown-toggle\"><icon type=\"caret-down\"></icon></div>\n" +
    "\t\t\t\t\t</div>\n" +
    "\t\t\t\t\t<div class=\"dropdown-menu\">\n" +
    "\t\t\t\t\t\t<div class=\"menu-item\" ng-click=\"selectFilters(selectedFilter.cat.label, true, true);\">Select All Visible</div>\n" +
    "\t\t\t\t\t\t<div class=\"menu-item\" ng-click=\"selectFilters(selectedFilter.cat.label, true, false);\">Deselect All Visible</div>\n" +
    "\t\t\t\t\t\t<div class=\"menu-item\" ng-click=\"selectFilters(selectedFilter.cat.label, false, false);\">Deselect All Not Visible</div>\n" +
    "\t\t\t\t\t</div>\n" +
    "\t\t\t\t</div>\n" +
    ...
    "\t\t\t\t</div>\n" +
    "\t\t\t</div>\n" +
    "\t\t\t<div class=\"list-box\" ng-style=\"{height: (windowHeight - 270) + 'px'}\">\n" +
